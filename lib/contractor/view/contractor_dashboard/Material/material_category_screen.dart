...
import 'package:ecoteam_app/admin/models/material_category_model.dart';
...
class _MaterialCategoryScreenState extends State<MaterialCategoryScreen> {
  ...
  Future<void> _refreshData() async {
    final provider = Provider.of<MaterialCategoryProvider>(
      context,
      listen: false,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: Consumer<CompanySiteProvider>(
        builder: (context, companyProvider, child) {
          if (!companyProvider.hasPermission('material-categories create')) {
  ...
  Widget _buildCategoryCard(
    ...
    MaterialCategoryProvider provider,
  ) {
    return Container(
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: Center(
                child: FaIcon(
                  ...
                  color: Color.fromARGB(255, 56, 85, 187),
                  size: 18,
                ),
              ),
            ),
            ...
            IconButton(
              ...
              onPressed: () =>
                  _showMaterialOptionsBottomSheet(context, category, provider),
            ),
          ],
        ),
      ),
                            ...
                            child: ElevatedButton(
                              onPressed: () async {
                                if (formKey.currentState!.validate()) {
                                  try {
                                    final provider =
                                        Provider.of<MaterialCategoryProvider>(
                                          context,
                                          listen: false,
                                        );
                                    ...
                                    if (isEditing) {
                                      await provider.updateCategory(
                                        ...
                                        // Use the method with dynamic IDs
                                        final companyProvider = Provider.of<CompanySiteProvider>(context, listen: false);
                                        await provider.addCategory(
                                          nameController.text.trim(),
                                          siteId: widget.siteId ?? int.tryParse(companyProvider.selectedCompanyId ?? '0') ?? 0,
                                          createdBy: widget.userId ?? companyProvider.currentUserId ?? 1,
                                          workspaceId: widget.workspaceId ?? int.tryParse(companyProvider.selectedCompanyId ?? '0') ?? 0,
                                        );
                                      }
  ...
  void _showMaterialOptionsBottomSheet(
    BuildContext context,
    MaterialCategory category,
    MaterialCategoryProvider materialProvider,
  ) {
    final companyProvider = Provider.of<CompanySiteProvider>(
      context,
      listen: false,
    );
    final canEdit = companyProvider.hasPermission('material-categories edit');
    final canDelete = companyProvider.hasPermission(
            ...
            children: [
              ...
              if (canDelete)
                _buildOptionItem(
                  ...
                  onTap: () {
                    Navigator.pop(sheetContext);
                    _showDeleteDialog(context, category, materialProvider);
                  },
                  isDestructive: true,
                ),
        ...
        child: Row(
          children: [
            Container(
              padding: EdgeInsets.all(12.w),
              decoration: BoxDecoration(
                color: backgroundColor ?? const Color(0xFF4a63c0).withOpacity(0.1),
