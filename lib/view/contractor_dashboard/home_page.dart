...
class HomePageApp extends StatelessWidget {
  ...
  home: const ContractorDashboardPage(),
);
  }
}
...
class ContractorDashboardPage extends StatefulWidget {
  ...
  @override
  State<ContractorDashboardPage> createState() => _ContractorDashboardPageState();
}

class _ContractorDashboardPageState extends State<ContractorDashboardPage> {
  List<String> companies = ['ABC Construction', 'XYZ Builders', 'Urban Developers', 'Infra Projects'];
  ...
  List<SiteData> sites = [
    ...
    SiteData(
      ...
      image: 'https://images.unsplash.com/photo-1581093450021-4a7360e9a7d3?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60',
      status: 'Planning',
      progress: 0.1,
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => DashboardScreen(
          selectedSite: site,
          companyName: currentCompany ?? 'Unknown Company',
        ),
      ),
    );
  }
  ...
  void _navigateToChatScreen() {
    final siteList = sites.map((site) => Site(
      id: site.id,
      name: site.name,
  ...
  void _showEditSiteDialog(SiteData site) {
    ...
    final TextEditingController addressController = TextEditingController(text: site.address);
    final TextEditingController startDateController = TextEditingController(text: site.startDate);
    final TextEditingController endDateController = TextEditingController(text: site.endDate);
                  ...
                  ElevatedButton(
                    onPressed: () {
                      if (nameController.text.isNotEmpty && addressController.text.isNotEmpty) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Site "${nameController.text}" updated successfully!'),
                            behavior: SnackBarBehavior.floating,
                          ),
                        );
                      }
