...
class _UnitManagementPageState extends State<UnitManagementPage> {
  ...
  Future<void> _loadUnits() async {
    try {
      ...
      print('DEBUG: Loading Units - WorkspaceID: $dynamicWorkspaceId, SiteID: 0');
    ...
    return Scaffold(
      ...
      floatingActionButton: Consumer<CompanySiteProvider>(
        builder: (context, provider, child) {
          if (!provider.hasPermission('material-unit create')) {
                ...
                Expanded(
                  child: Consumer<CompanySiteProvider>(
                    builder: (context, siteProvider, _) {
                      if (!siteProvider.hasPermission('material-unit show')) {
                        return Center(
                           child: Text(
                            'Access Denied',
                            style: TextStyle(
                      ...
                      return provider.filteredUnits.isEmpty
  ...
  Widget _buildUnitsList(UnitProvider provider) {
    return ListView.builder(
      padding: EdgeInsets.fromLTRB(16.w, 16.h, 16.w, 107.h),
        ...
        child: Row(
          ...
          children: [
            ...
            IconButton(
              ...
              color: const Color(0xFF4a63c0),
              iconSize: 20.sp,
              onPressed: () => _showUnitOptionsBottomSheet(context, unit, provider),
            ),
          ],
        ),
      ),
                                          ...
                                          try {
                                            ...
                                            if (isEditing) {
                                              ...
                                              final newUnit = Unit(
                                                ...
                                                description: '',
                                                isActive: true,
                                                siteId: widget.siteId ?? int.tryParse(companyProvider.selectedCompanyId ?? '0') ?? 1,
                                                workspaceId: widget.workspaceId ?? int.tryParse(companyProvider.selectedCompanyId ?? '0') ?? 1,
  ...
  void _showUnitOptionsBottomSheet(
    BuildContext context,
    Unit unit,
    UnitProvider provider,
  ) {
            ...
            children: [
              ...
              Consumer<CompanySiteProvider>(
                builder: (context, siteProvider, child) {
                  if (!siteProvider.hasPermission('material-unit edit')) {
                    return const SizedBox.shrink();
                  }
              ...
              Consumer<CompanySiteProvider>(
                builder: (context, siteProvider, child) {
                  if (!siteProvider.hasPermission('material-unit delete')) {
                    return const SizedBox.shrink();
                  }
                  return _buildOptionItem(
                    context,
